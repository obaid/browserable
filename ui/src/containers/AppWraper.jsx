  ...
  if (token) {
    return (
      <div className="w-full h-full flex flex-col flex-grow">
        <div className="w-full h-full flex flex-grow max-h-screen max-w-screen" >
          <div
            className={`${
              sidebarIsOpen
                ? "w-full sm:w-64 pb-4 pt-16 sm:pt-4 absolute sm:relative top-0 bottom-0 left-0 right-0 bg-white z-10"
                : "w-64 hidden sm:visible sm:flex"
          ...
          <div className="flex-grow flex flex-col max-h-screen" style={{
            maxWidth: !isMobile ? "calc(100% - 16rem)" : "100%"
          }}>
            {/* children go here */}
            {props.children}
          </div>
